...
function AnalyseData() {
    cnacc_data=($(cat ../data/data_cnacc.txt | grep "\@" | tr -d "\@" > ./cnacc_addition.tmp && cat ../data/data_gfwlist.txt | grep "\@" | tr -d "\@" > ./gfwlist_addition.tmp && cat ../data/data_cnacc.txt | grep "\!" | tr -d "\!" > ./cnacc_subtraction.tmp && cat ../data/data_gfwlist.txt | grep "\!" | tr -d "\!" > ./gfwlist_subtraction.tmp && cat ./split_other.tmp | grep "\@cn" | sed "s/\:\@cn//g;s/domain\://g;s/full\://g" >> ./cnacc_other.tmp && cat ./split_other.tmp | grep -v "\@cn" | sed "s/domain\://g;s/full\://g" >> ./gfwlist_other.tmp && cat ./cnacc_domain.tmp ./cnacc_other.tmp ../data/data_cnacc.txt | sed "s/\:\@cn//g;s/\/114\.114\.114\.114//g;s/server\=\///g;s/domain\://g;s/full\://g" | tr "A-Z" "a-z" | grep -E "^(([a-z]{1})|([a-z]{1}[a-z]{1})|([a-z]{1}[0-9]{1})|([0-9]{1}[a-z]{1})|([a-z0-9][-_\.a-z0-9]{1,61}[a-z0-9]))\.([a-z]{2,13}|[a-z0-9-]{2,30}\.[a-z]{2,3})$" | sort | uniq > ./cnacc_checklist.tmp && cat ./gfwlist_base64.tmp ./gfwlist_domain.tmp ./gfwlist_other.tmp ../data/data_gfwlist.txt | sed "s/domain\://g;s/full\://g;s/http\:\/\///g;s/https\:\/\///g" | tr -d "|" | tr "A-Z" "a-z" | grep -E "^(([a-z]{1})|([a-z]{1}[a-z]{1})|([a-z]{1}[0-9]{1})|([0-9]{1}[a-z]{1})|([a-z0-9][-_\.a-z0-9]{1,61}[a-z0-9]))\.([a-z]{2,13}|[a-z0-9-]{2,30}\.[a-z]{2,3})$" | sort | uniq > gfwlist_checklist.tmp && awk 'NR == FNR { tmp[$0] = 1 } NR > FNR { if ( tmp[$0] != 1 ) print }' ./dead_domain.tmp ./cnacc_checklist.tmp > ./cnacc_alive.tmp && awk 'NR == FNR { tmp[$0] = 1 } NR > FNR { if ( tmp[$0] != 1 ) print }' ./dead_domain.tmp ./gfwlist_checklist.tmp > ./gfwlist_alive.tmp && cat ./cnacc_alive.tmp ./cnacc_addition.tmp | sort | uniq > ./cnacc_added.tmp && cat ./gfwlist_alive.tmp ./gfwlist_addition.tmp | sort | uniq > ./gfwlist_added.tmp && awk 'NR == FNR { tmp[$0] = 1 } NR > FNR { if ( tmp[$0] != 1 ) print }' ./cnacc_subtraction.tmp ./cnacc_added.tmp > ./cnacc_subtracted.tmp && awk 'NR == FNR { tmp[$0] = 1 } NR > FNR { if ( tmp[$0] != 1 ) print }' ./gfwlist_subtraction.tmp ./gfwlist_added.tmp > ./gfwlist_subtracted.tmp && cat ./cnacc_subtracted.tmp | rev | cut -d "." -f 1,2 | rev | sort | uniq > ./lite_cnacc_data.tmp && cat ./gfwlist_subtracted.tmp | rev | cut -d "." -f 1,2 | rev | sort | uniq > ./lite_gfwlist_data.tmp && cat ./cnacc_subtracted.tmp ./lite_cnacc_data.tmp | sort | uniq > ./cnacc_data.tmp && cat ./gfwlist_subtracted.tmp ./lite_gfwlist_data.tmp | sort | uniq > ./gfwlist_data.tmp && awk 'NR == FNR { tmp[$0] = 1 } NR > FNR { if ( tmp[$0] != 1 ) print }' ./gfwlist_data.tmp ./cnacc_data.tmp | awk "{ print $2 }"))
