...
function AnalyseData() {
    cnacc_data=($(cat ../data/data_cnacc.txt | grep "\@" | tr -d "@" > ./cnacc_forced.tmp && cat ../data/data_gfwlist.txt | grep "\@" | tr -d "@" > ./gfwlist_forced.tmp && cat ./cnacc_domain.tmp ../data/data_cnacc.txt | sed "s/\/114\.114\.114\.114//g;s/server\=\///g" | tr "A-Z" "a-z" | grep -E "^(([a-z]{1})|([a-z]{1}[a-z]{1})|([a-z]{1}[0-9]{1})|([0-9]{1}[a-z]{1})|([a-z0-9][-_\.a-z0-9]{1,61}[a-z0-9]))\.([a-z]{2,13}|[a-z0-9-]{2,30}\.[a-z]{2,3})$" | sort | uniq > ./cnacc_data.tmp && awk 'NR == FNR { tmp[$0] = 1 } NR > FNR { if ( tmp[$0] != 1 ) print }' ./dead_domain.tmp ./cnacc_data.tmp > ./cnacc_alive.tmp && cat ./gfwlist_base64.tmp ./gfwlist_domain.tmp ../data/data_gfwlist.txt | tr -d "|" | tr "A-Z" "a-z" | grep -E "^(([a-z]{1})|([a-z]{1}[a-z]{1})|([a-z]{1}[0-9]{1})|([0-9]{1}[a-z]{1})|([a-z0-9][-_\.a-z0-9]{1,61}[a-z0-9]))\.([a-z]{2,13}|[a-z0-9-]{2,30}\.[a-z]{2,3})$" | sort | uniq > gfwlist_data.tmp && awk 'NR == FNR { tmp[$0] = 1 } NR > FNR { if ( tmp[$0] != 1 ) print }' ./dead_domain.tmp ./gfwlist_data.tmp > ./gfwlist_alive.tmp && cat ./cnacc_alive.tmp ./cnacc_forced.tmp | sort | uniq > ./cnacc_final.tmp && cat ./gfwlist_alive.tmp ./gfwlist_forced.tmp | sort | uniq > ./gfwlist_final.tmp && awk 'NR == FNR { tmp[$0] = 1 } NR > FNR { if ( tmp[$0] != 1 ) print }' ./gfwlist_final.tmp ./cnacc_final.tmp | awk "{ print $2 }"))
    gfwlist_data=($(awk 'NR == FNR { tmp[$0] = 1 } NR > FNR { if ( tmp[$0] != 1 ) print }' ./cnacc_final.tmp ./gfwlist_final.tmp | awk "{ print $2 }"))
    lite_cnacc_data=($(cat ./cnacc_final.tmp | rev | cut -d "." -f 1,2 | rev | sort | uniq > lite_cnacc_final.tmp && cat ./gfwlist_final.tmp | rev | cut -d "." -f 1,2 | rev | sort | uniq > lite_gfwlist_final.tmp && awk 'NR == FNR { tmp[$0] = 1 } NR > FNR { if ( tmp[$0] != 1 ) print }' ./lite_gfwlist_final.tmp ./lite_cnacc_final.tmp | awk "{ print $2 }"))
    lite_gfwlist_data=($(awk 'NR == FNR { tmp[$0] = 1 } NR > FNR { if ( tmp[$0] != 1 ) print }' ./lite_cnacc_final.tmp ./lite_gfwlist_final.tmp | awk "{ print $2 }"))
}
# File Switch
function FileSwitch() {
